...
function req(body?: unknown, url = "http://localhost/api/users") {
  return { url, json: jest.fn().mockResolvedValue(body) } as unknown as NextRequest;
}
...
describe("GET /api/users", () => {
  ...
  it("200 lista usuarios", async () => {
    ...
    const users = [{ id: 1, name: "John", email: "j@j.com", role: "ADMIN", status: true, created_at: "2024-01-01" }];
    let callIdx = 0;
...
describe("POST /api/users", () => {
  ...
  it("401 nao autenticado", async () => {
    (auth as jest.Mock).mockResolvedValueOnce(null);
    const res = await postUser(req({ name: "J", email: "j@j.com", password: "pass123", role: "USER" }));
    expect(res.status).toBe(401);
  });
  ...
  it("400 role invalido", async () => {
    (auth as jest.Mock).mockResolvedValueOnce({ user: { role: "ADMIN" } });
    const res = await postUser(req({ name: "J", email: "j@j.com", password: "pass123", role: "INVALID" }));
    expect(res.status).toBe(400);
  });

  it("400 senha curta", async () => {
    (auth as jest.Mock).mockResolvedValueOnce({ user: { role: "ADMIN" } });
    const res = await postUser(req({ name: "J", email: "j@j.com", password: "123", role: "USER" }));
    expect(res.status).toBe(400);
  });
  ...
  it("201 criado com sucesso", async () => {
    ...
    mockSql.mockResolvedValueOnce([{ id: 1, name: "J", email: "j@j.com", role: "USER", created_at: "2024-01-01" }]);
    const res = await postUser(req({ name: "J", email: "j@j.com", password: "pass123", role: "USER" }));
...
describe("GET /api/users/[id]", () => {
  ...
  it("403 sem permissao", async () => {
    (auth as jest.Mock).mockResolvedValueOnce({ user: { id: "1", role: "USER" } });
  ...
  it("200 admin acessa usuario", async () => {
    (auth as jest.Mock).mockResolvedValueOnce({ user: { id: "1", role: "ADMIN" } });
    const user = { id: 1, name: "J", email: "j@j.com", role: "USER", status: true, created_at: "2024-01-01" };
  ...
  it("200 proprio usuario", async () => {
    (auth as jest.Mock).mockResolvedValueOnce({ user: { id: "1", role: "OPERATOR" } });
    mockSql.mockResolvedValueOnce([{ role: "OPERATOR" }]);
    const user = { id: 1, name: "J", email: "j@j.com", role: "USER", status: true, created_at: "2024-01-01" };
    mockSql.mockResolvedValueOnce([user]);
    const res = await getUserById(req(), { params: Promise.resolve({ id: "1" }) });
...
describe("PUT /api/users/[id]", () => {
  beforeEach(() => { mockSql.mockReset(); (sql as any).unsafe = jest.fn(); jest.clearAllMocks(); });

  it("401 nao autenticado", async () => {
    (auth as jest.Mock).mockResolvedValueOnce(null);
    const res = await putUser(req({ name: "Updated" }), { params: Promise.resolve({ id: "1" }) });
    expect(res.status).toBe(401);
  });
  ...
  it("403 sem permissao para ADMIN", async () => {
    ...
    const res = await putUser(req({ role: "ADMIN" }), { params: Promise.resolve({ id: "2" }) });
    expect(res.status).toBe(403);
  });

  it("400 role invalido", async () => {
    (auth as jest.Mock).mockResolvedValueOnce({ user: { id: "1", role: "ADMIN" } });
    const res = await putUser(req({ role: "INVALID" }), { params: Promise.resolve({ id: "1" }) });
    expect(res.status).toBe(400);
  });
  ...
  it("200 atualizado com sucesso", async () => {
    ...
    (sql as any).unsafe = jest.fn().mockResolvedValueOnce([{ id: 1, name: "Updated", email: "j@j.com", role: "USER", status: true, created_at: "2024-01-01" }]);
    const res = await putUser(req({ name: "Updated", password: "newpass123" }), { params: Promise.resolve({ id: "1" }) });
    expect(res.status).toBe(200);
  });
});
