...
describe('Post /api/login', () => {
    ...
    it ('Deve retornar 403 se o usuario está inativo', async () => {
        mockSingleFn.mockResolvedValueOnce({
            data: {id: 1, email: 'test@example.com', status: false, password: 'hashed_password'},
            error: null
        });
    ...
    it('Deve retornar 401 se a senha não combinar', async () => {
        mockSingleFn.mockResolvedValueOnce ({
            data: {id: 1, email: 'test@example.com', status: true, password: 'hashed_password'},
            error: null
        });
        (bcrypt.compare as jest.Mock).mockResolvedValueOnce(false);
        const req = createMockRequest({email: 'test@example.com', password: 'wrongpasswprd'})
        const res = await POST(req)
        const json = await res.json()
