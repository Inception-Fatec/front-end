...
export function GlobalTour() {
  const { isActive, currentStep, nextStep, closeTour, isNavigating } = useTour();
      ...
      if (el) {
        ...
        if (newRectString !== lastRectString) {
          ...
          if (currentStep.placement === 'overlay') {
            cTop = sTop + GAP_Y; 
            if (cTop < 16) cTop = 16;
            if (cTop + cardHeight > viewportHeight - 16) cTop = viewportHeight - cardHeight - 16;
          } 
          ...
          if (cLeft < 16) cLeft = 16;
          if (cLeft + CARD_WIDTH > viewportWidth - 16) cLeft = viewportWidth - CARD_WIDTH - 16;
