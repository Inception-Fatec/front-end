...
import type { Grouping, GroupingWithStations, GroupingWithStationDetails } from "@/types/grouping";
...
export async function POST(req: NextRequest) {
    ...
    try {
        const body: Pick<Grouping, "name"> & { stations?: number[] } = await req.json();
        ...
        if (Array.isArray(stations) && stations.length > 0) {
            ...
            if (relationError) {
                ...
                return NextResponse.json({ error: "Erro ao vincular estações ao grupo." }, { status: 500 });
            }
        }
        ...
        if (fetchError || !groupWithStations) {
            return NextResponse.json({ error: "Erro ao buscar grupo criado." }, { status: 500 });
        }

        return NextResponse.json(groupWithStations as GroupingWithStations, { status: 201 });
...
export async function GET(req: NextRequest) {
    ...
    try {
      ...
      `)
        ...
        return NextResponse.json(allGroups as GroupingWithStationDetails[], { status: 200 });
...
export async function PUT(req: NextRequest) {
    ...
    try {
        const body: Partial<Grouping> & { id: number; stations?: number[] } = await req.json();
        ...
        if (name) {
            ...
            if (updateError) {
                ...
                return NextResponse.json({ error: "Erro ao atualizar nome do grupo." }, { status: 500 });
            }
        }
