...
export async function GET(req: NextRequest) {
    ...
    try {
        if (id) {
            ...
            // Busca o último measurement de cada parâmetro separadamente
            const parametersWithMeasurements = await Promise.all(
                station.parameters.map(async (param: { id: number; id_parameter_type: number; status: boolean; parameter_types: { name: string; unit: string } }) => {
            ...
            return NextResponse.json(
                { ...station, parameters: parametersWithMeasurements } as StationWithDetails,
                { status: 200 },
            );
        }
